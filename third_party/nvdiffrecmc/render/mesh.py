...
class Mesh:
    ...
    #  moving vertices for campos optimization
    def move_mesh(self, q, t, scale):
        self.v_pos = self.v_pos * scale
        self.v_pos = rotate_vector(q, self.v_pos) + t.reshape(1, -1)
        # self.v_pos = torch.mm(R, self.v_pos.T).T + t.reshape(1, -1) # for rotation matrix


######################################################################################
# rotate using quaternions
######################################################################################
# reference: https://graphics.stanford.edu/courses/cs348a-17-winter/Papers/quaternion.pdf
def multiply_quats(q1, q2, N):
    q1 = q1.reshape(-1, 4)
    q2 = q2.reshape(-1, 4)

    prod = torch.zeros(N, 4, device='cuda')
    prod[:, 0] = q1[:, 0] * q2[:, 0] - torch.sum(q1[:, 1:] * q2[:, 1:], dim=1)
    prod[:, 1:] += q1[:, 0].reshape(-1, 1) * q2[:, 1:]
    prod[:, 1:] += q2[:, 0].reshape(-1, 1) * q1[:, 1:]
    prod[:, 1:] += torch.linalg.cross(q1[:, 1:], q2[:, 1:])
    return prod

# q: unit rotation quaternion, v: N x 3 vertex position matrix
def rotate_vector(q, v):
    q_inv = q.clone()
    q_inv[1:] = -q_inv[1:]
    N = v.shape[0]
    q_v_pos = torch.column_stack((torch.zeros(N, device='cuda'), v))
    q_v_pos = multiply_quats(multiply_quats(q, q_v_pos, N), q_inv, N)
    v_pos = q_v_pos[:, 1:]
    return v_pos
