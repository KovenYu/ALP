...
######################################################################################
# Environment map smoothless loss
######################################################################################
def env_smoothness_grad(envmap):
    assert (envmap.dim() == 3)
    offset = torch.normal(mean=0, std=0.005, size=(envmap.shape[0], envmap.shape[1], 2), device="cuda")
    jitter = (util.pixel_grid(envmap.shape[1], envmap.shape[0])[None, ...] + offset).contiguous()
    envmap_jitter = dr.texture(envmap.contiguous().unsqueeze(0), jitter, filter_mode='linear', boundary_mode='clamp')
    loss = torch.mean(torch.abs(envmap_jitter - envmap))
    return loss
