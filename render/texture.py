...
def create_trainable(init, res=None, auto_mipmaps=True, min_max=None, requires_grad=True):
    with torch.no_grad():
        ...
        if not auto_mipmaps:
            mip_chain = [init.clone().detach().requires_grad_(requires_grad)]
            ...
            return Texture2D(init.clone().detach().requires_grad_(requires_grad), min_max=min_max)
