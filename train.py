...
def prepare_batch(target, train_res, bg_type):
    ...
    if 'convert_frame' in target.keys():
        target['convert_frame'] = target['convert_frame'].cuda()
...
def initial_guess_material(geometry, mlp, FLAGS, init_mat=None):
    ...
    if mlp:
        ...
        if init_mat is None:
            ...
            kd_map_opt = texture.create_trainable(kd_init , FLAGS.texture_res, not FLAGS.custom_mip, [kd_min, kd_max], requires_grad=requires_grad)
            ...
            ks_map_opt = texture.create_trainable(np.concatenate((ksR, ksG, ksB), axis=2), FLAGS.texture_res, not FLAGS.custom_mip, [ks_min, ks_max], requires_grad=requires_grad)
        else:
            kd_map_opt = texture.create_trainable(init_mat['kd'], FLAGS.texture_res, not FLAGS.custom_mip, [kd_min, kd_max], requires_grad=requires_grad)
            ks_map_opt = texture.create_trainable(init_mat['ks'], FLAGS.texture_res, not FLAGS.custom_mip, [ks_min, ks_max], requires_grad=requires_grad)

        # Setup normal map
        if init_mat is None or 'normal' not in init_mat:
            normal_map_opt = texture.create_trainable(np.array([0, 0, 1]), FLAGS.texture_res, not FLAGS.custom_mip, [nrm_min, nrm_max], requires_grad=requires_grad)
        else:
            normal_map_opt = texture.create_trainable(init_mat['normal'], FLAGS.texture_res, not FLAGS.custom_mip, [nrm_min, nrm_max], requires_grad=requires_grad)
...
def validate_itr(glctx, target, ref_mesh, geometry, opt_material, lgt, FLAGS, denoiser, iter=0):
    ...
    with torch.no_grad():
        ...
        buffers = render.render_mesh(FLAGS, glctx, opt_mesh, target['mvp'], target['campos'], target['light'] if lgt is None else lgt, target['resolution'],
                                        ...
                                        denoiser=denoiser, transform=transform)

        result_dict['ref'] = target['img'][0, ...,0:3]  # linear rgb
        result_dict['opt'] = torch.log(torch.clamp(buffers['shaded'][...,0:3], min=0, max=65535) + 1)[0]  # linear rgb
        ...
        if FLAGS.display is not None:
            ...
            for layer in FLAGS.display:
                if 'latlong' in layer and layer['latlong']:
                    ...
                    if layer['bsdf'] == 'kd':
                        result_dict[layer['bsdf']] = img[..., 0:3][0]  # linear rgb
                    else:
                    ...
                    if ref_mesh is not None:
                        img = render.render_mesh(FLAGS, glctx, ref_mesh, target['mvp'], target['campos'], target['light'], target['resolution'],
                                                    spp=target['spp'], num_layers=FLAGS.layers, background=white_bg, bsdf=layer['bsdf'], optix_ctx=geometry.optix_ctx)['shaded']
                        if layer['bsdf'] == 'kd':
                            result_dict[layer['bsdf'] + "_ref"] = img[..., 0:3][0]  # linear rgb
                        else:
        ...
        normal = result_dict['normal']  # [H, W, 3], ranging [0, 1]
        normal_ = normal * 2 - 1  # [H, W, 3], ranging [-1, 1]
        world2cam = target['mv'].squeeze(0)  # [4, 4]
        normal_cam = torch.matmul(world2cam[:3, :3], normal_[..., None]).squeeze(-1)  # [H, W, 3]
        normal_cam = util.safe_normalize(normal_cam)
        result_dict['normal'] = normal_cam / 2 + 0.5  # [H, W, 3], ranging [0, 1]

        return result_image, result_dict, buffers

def validate(glctx, geometry, opt_material, lgt, dataset_validate, out_dir, FLAGS, denoiser, save_validation=False):
    ...
    FLAGS.n_samples = 32
    denoiser = None
    FLAGS.train_res = FLAGS.val_res
    FLAGS.display_res = FLAGS.val_res
    ...
    with open(os.path.join(out_dir, 'metrics.txt'), 'w') as fout:
        ...
        for it, target in enumerate(dataloader_validate):
            ...
            result_image, result_dict, buffers = validate_itr(glctx, target, dataset_validate.getMesh(), geometry, opt_material, lgt, FLAGS, denoiser)
            ...
            for k in result_dict.keys():
                np_img = (result_dict[k] * buffers['shaded'][..., 3].permute(1, 2, 0)).detach().cpu().numpy()
                mask = buffers['shaded'][..., 3].permute(1, 2, 0).detach().cpu().numpy()
                util.save_image_raw(out_dir + '/' + ('mask.exr'), mask)

                if save_validation and k == "normal":
                    util.save_image_raw(out_dir + '/' + ('%s.exr' % (k)), np_img)
                elif save_validation and k == "ks":
                    util.save_image_raw(out_dir + '/' + ('%s.exr' % (k)), np_img)
                    util.save_image_raw(out_dir + '/' + ('%s_m.exr' % (k)), np_img[:, :, 2])
                    util.save_image_raw(out_dir + '/' + ('%s_r.exr' % (k)), np_img[:, :, 1])
                    util.save_image_raw(out_dir + '/' + ('%s_o.exr' % (k)), np_img[:, :, 0])
...
def optimize_mesh(
    ...
    if optimize_light and not FLAGS.only_camera_pose:
    ...
    for it, target in enumerate(dataloader_train):
        ...
        if display_image or save_image:
            result_image, result_dict, _ = validate_itr(glctx_display, prepare_batch(next(v_it), FLAGS.train_res, FLAGS.background),
        ...
        if optimize_geometry:
            if FLAGS.opt_cam_pos and it < 1000:
                optimizer_mesh.step()
                scheduler_mesh.step()
            elif not FLAGS.opt_cam_pos:
                optimizer_mesh.step()
                scheduler_mesh.step()
        ...
        if log_interval and (it % log_interval == 0):
            ...
            lr_pose = optimizer_mesh.param_groups[0]['lr']
            lr_light = optimizer_light.param_groups[0]['lr']
            print("iter=%5d, img_loss=%.6f, reg_loss=%.6f, lr_pose=%.5f, lr_light=%.5f, rem=%s" %
                (it, img_loss_avg, reg_loss_avg, lr_pose, lr_light, util.time_to_text(remaining_time)))

            # if FLAGS.opt_cam_pos:
            #     print(geometry.rot, geometry.trans, geometry.scale)
...
if __name__ == "__main__":
    ...
    FLAGS.opt_cam_pos         = False                    # lockpos needs to be False for this to work. Optimizes camera pose but not vertex positions.
    FLAGS.run_proposed        = False
    FLAGS.rot_init            = [1, 0, 0, 0]
    FLAGS.smooth_envmap       = False
    FLAGS.smooth_envmap_wt    = 0.01
    FLAGS.only_camera_pose    = False
    FLAGS.ablation = False
    FLAGS.save_validation = None
    FLAGS.camera_space_light = None
    FLAGS.nimages = None
    FLAGS.convert_frame = None
    FLAGS.no_texture_opt = False
    ...
    if FLAGS.out_dir is None:
        ...
        FLAGS.out_dir = 'out/ablation/'  + FLAGS.out_dir
    elif FLAGS.save_validation:
        pass
    else:
    ...
    if FLAGS.learn_lighting:
        lgt = light.create_trainable_env_rnd(FLAGS.probe_res, scale=0.0, bias=0.5, FLAGS=FLAGS)
        if FLAGS.only_camera_pose:
            lgt.base.requires_grad_(False)
    else:
    ...
    if FLAGS.base_mesh is None:
        ...
        import copy
        mat_pose = copy.deepcopy(mat)
        mat_pose['ks'].data[..., 1] = mat_pose['ks'].data[..., 1] * 0 + 0.35

        geometry, mat_pose = optimize_mesh(denoiser, glctx, glctx_display, geometry, mat_pose, lgt, dataset_train, dataset_validate, FLAGS,
    ...
    # ==============================================================================================
    if FLAGS.validate:
        avg_psnr = validate(glctx_display, geometry, mat, lgt, dataset_validate, os.path.join(FLAGS.out_dir, "validate"), FLAGS, denoiser, FLAGS.save_validation)
    ...
    if FLAGS.save_validation:
        import pickle
        fname = os.path.join(FLAGS.out_dir, 'val.pkl')
